...
## Health + Death foundation coverage (Godot-only, Save Version 10+).
## Sections: health defaults/clamps, deprivation damage with exact hourly
...
static func _save_v10_round_trip(h: TestHarness) -> void:
	...
	h.eq_int("Save-V10 version is exactly 10", raw["Version"], 10)
